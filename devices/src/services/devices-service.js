...
class DevicesService {
    ...
    async createDevice(userInputs) {
        ...
        const { userId, groupId, name, type, channels, code, apiKey, shellyUrl } = userInputs;

        const device = await this.repository.createDevice(userId, groupId, name, type, channels, code, apiKey, shellyUrl);
    ...
    async updateDevice(userInputs, userId) {
        const { id, groupId, name, type, channels, channelsDelays, code, apiKey } = userInputs;
        const device = await this.repository.updateDevice({ id, groupId, name, type, channels, channelsDelays, code, apiKey, userId });
        if (!device) throw new NotFoundError('device not found');
        return device;
    }

    async updateDevicesWithGroupId(userInputs) {
        const { groupId, devices } = userInputs;
        const device = await this.repository.updateDevicesWithGroupId(groupId, devices);
        if (!device) throw new NotFoundError('device not found');
        return device;
    }
